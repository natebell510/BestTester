...
async function resolveBuildNumber(queueId: number): Promise<number> {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    await sleep(5_000);
    const res = await axios.get(`${JENKINS_URL}/queue/item/${queueId}/api/json`, {
      auth: jenkinsAuth,
    });
...
async function main() {
  ...
  if (!JENKINS_URL) {
    console.error('❌ JENKINS_URL not set');
    process.exit(1);
  }
  if (!jiraConfig.baseUrl) {
    console.error('❌ JIRA_BASE_URL not set');
    process.exit(1);
  }
  ...
  await postMessage(
    `🚀 *BestTester* triggered on Jenkins (suite: *${SUITE}*). Queue id: ${queueId}`,
  );
  ...
  for (let i = 0; i < 120; i++) {
    try {
      ...
      if (chunk) {
        ...
        logOffset += Buffer.byteLength(chunk);
      }
      const more = logRes.headers['x-more-data'];
      if (more !== 'true') break;
    } catch {
      /* build may not have started logging yet */
    }
    // eslint-disable-next-line no-restricted-syntax
    await sleep(10_000);
  ...
  await postMessage(
    `${icon} *BestTester* build #${buildNumber} finished: *${status}*\n${buildUrl}`,
  );
  ...
  const junitOk = await downloadArtifact(
    buildNumber,
    'reports/playwright-report/junit.xml',
    junitDest,
  );
  const jsonOk = await downloadArtifact(
    buildNumber,
  ...
  const execution = await jira.createTestExecution(
    {
      summary: `BestTester Playwright Run — Build #${buildNumber}`,
      junitPath,
      tests: allTests,
      buildUrl,
    },
  ...
  if (JIRA_KEY) {
    ...
    if (result.created.length)
      commentLines.push(`🐛 New bugs created: ${result.created.join(', ')}`);
