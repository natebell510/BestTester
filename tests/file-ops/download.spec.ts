...
import { verifyFileExists, verifyFileSize, verifyExcelDownload, verifyPDFDownload } from '../../src/utils/download-verifier';
...
test.describe('Download Verification @regression @file-ops', () => {
  test('should verify Excel file exists and has correct headers', async () => {
    ...
    await writeExcel(filePath, [{ 'First Name': 'John', 'Last Name': 'Doe' }], ['First Name', 'Last Name']);
  ...
  test('should verify PDF file contains expected keywords', async () => {
    ...
    generatePDF(filePath, 'Employee Report: John Doe - Annual Leave approved.');
